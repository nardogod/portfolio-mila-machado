...
const HomePage: NextPage = () => {
  ...
  // Efeito para navegar automaticamente após 5 segundos
  useEffect(() => {
    const autoNavigateTimer = setTimeout(() => {
      setIsFadingOut(true);
      // Espera a animação de fade-out (500ms) terminar antes de navegar
      setTimeout(() => {
  ...
  useEffect(() => {
    ...
    const titleText =
      countdown > 0
        ? `Acessando meu mundo em ${countdown} segundos...`
        : "Teleportando...";
    const fontSize = Math.min(canvas.width / 12, 70); // Tamanho da fonte responsivo
    ctx.font = `bold ${fontSize}px 'Orbitron', sans-serif`;
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillStyle = "#ebe709ff"; // Cor branca para o texto
  ...
  }, [countdown]); // Redesenha o canvas quando a contagem mudar
